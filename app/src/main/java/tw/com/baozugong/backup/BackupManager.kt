...
class BackupManager(private val db: AppDatabase) {
    ...
    suspend fun restore(resolver: ContentResolver, uri: Uri, password: String) {
        val root = read(resolver, uri, password)
        require(root.getInt("version") == 1) { "不支援此備份版本" }
        db.withTransaction {
            dao.clearAll()
            ...
            dao.restoreLeases(root.array("leases").objects().map { Lease(it.long("id"),it.long("roomId"),it.long("tenantId"),it.str("startDate"),it.str("endDate"),it.long("monthlyRent"),it.int("dueDay"),it.long("deposit"),it.str("note"),it.str("status"),it.optString("endedAt").ifBlank { null }) })
    ...
    private suspend fun snapshot(): JSONObject = JSONObject().apply {
        put("version", 1); put("createdAt", LocalDateTime.now().toString())
        ...
        put("leases", JSONArray(dao.allLeases().map { JSONObject().apply { put("id",it.id);put("roomId",it.roomId);put("tenantId",it.tenantId);put("startDate",it.startDate);put("endDate",it.endDate);put("monthlyRent",it.monthlyRent);put("dueDay",it.dueDay);put("deposit",it.deposit);put("note",it.note);put("status",it.status);put("endedAt",it.endedAt ?: "") } }))
