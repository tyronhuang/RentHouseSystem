...
class BackupManager(private val db: AppDatabase) {
    ...
    suspend fun restore(resolver: ContentResolver, uri: Uri, password: String) {
        val root = read(resolver, uri, password)
        require(root.getInt("version") in 1..3) { "不支援此備份版本" }
        db.useWriterConnection { connection ->
            connection.immediateTransaction {
                ...
                dao.restorePayments(root.array("payments").objects().map {
                    Payment(
                        id = it.long("id"), invoiceId = it.long("invoiceId"), amount = it.long("amount"),
                        paidDate = it.str("paidDate"), method = it.str("method"), note = it.str("note"),
                        createdAt = it.str("createdAt"), updatedAt = it.str("updatedAt"),
                        voided = it.optBool("voided"), voidedAt = it.optString("voidedAt").ifBlank { null },
                    )
                })
            }
        }
    ...
    private suspend fun snapshot(): JSONObject = JSONObject().apply {
        put("version", 3); put("createdAt", LocalDateTime.now().toString())
        ...
        put("payments", JSONArray(dao.allPayments().map { JSONObject().apply { put("id",it.id);put("invoiceId",it.invoiceId);put("amount",it.amount);put("paidDate",it.paidDate);put("method",it.method);put("note",it.note);put("createdAt",it.createdAt);put("updatedAt",it.updatedAt);put("voided",it.voided);put("voidedAt",it.voidedAt ?: "") } }))
    }
...
private fun JSONObject.optBool(name:String)=if(has(name)) optBoolean(name,false) else false
