...
fun PeopleScreen(vm:AppViewModel,onMessage:(String)->Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(bottom=24.dp)) {
        ...
        if(!showTenants) {
            ...
            items(tenants,key={it.id}) { tenant -> ListItem(headlineContent={Text(tenant.name)},supportingContent={Text(listOf(PhoneRules.formatStored(tenant.phone),if(tenant.lineName.isBlank())"" else "LINE ${tenant.lineName}").filter{it.isNotBlank()}.joinToString(" · ").ifBlank{"未填聯絡方式"})},trailingContent={Text("編輯")},modifier=Modifier.clickable{editTenant=tenant});HorizontalDivider() }
        }
    }
...
@Composable private fun LeaseCard(value:LeaseListRow,onCall:(()->Unit)?,onEdit:(()->Unit)?,onRenew:(()->Unit)?,onEnd:(()->Unit)?) {
    Card(Modifier.fillMaxWidth().padding(horizontal=16.dp,vertical=6.dp)){Column(Modifier.padding(16.dp)){Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Text("${value.venueName} · ${value.roomName}",fontWeight=FontWeight.Bold);Text(if(value.status==LeaseStatus.ACTIVE)"有效" else "已退租",color=MaterialTheme.colorScheme.primary)};Text("${value.tenantName}　${PhoneRules.formatStored(value.tenantPhone)}");Text("${value.startDate} ～ ${value.endDate}");Text("月租 ${money(value.monthlyRent)}　押金 ${money(value.deposit)}　每月 ${value.dueDay} 日繳");if(value.waterFee+value.managementFee+value.electricityFee>0)Text("水費 ${money(value.waterFee)}　管理費 ${money(value.managementFee)}　電費 ${money(value.electricityFee)}",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant);if(onCall!=null||onEdit!=null||onRenew!=null||onEnd!=null)Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.End){if(onCall!=null)TextButton(onClick=onCall){Text("電話")};if(onEdit!=null)TextButton(onClick=onEdit){Text("修改")};if(onRenew!=null)TextButton(onClick=onRenew){Text("續約")};if(onEnd!=null)TextButton(onClick=onEnd){Text("退租")}}}}
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable private fun NewLeaseDialog(rooms:List<RoomListRow>,onDismiss:()->Unit,onSave:(Tenant,Lease)->Unit) {
    var roomId by remember{mutableLongStateOf(rooms.firstOrNull()?.id?:0)};var expanded by remember{mutableStateOf(false)};var name by remember{mutableStateOf("")};var phoneTail by remember{mutableStateOf("")};var line by remember{mutableStateOf("")};var start by remember{mutableStateOf(LocalDate.now().toString())};var end by remember{mutableStateOf(LocalDate.now().plusYears(1).minusDays(1).toString())};val selected=rooms.firstOrNull{it.id==roomId};var rent by remember(selected?.id){mutableStateOf((selected?.defaultRent?:0).toString())};var due by remember{mutableStateOf("5")};var deposit by remember{mutableStateOf("")};var waterFee by remember{mutableStateOf("")};var managementFee by remember{mutableStateOf("")};var electricityFee by remember{mutableStateOf("")};var note by remember{mutableStateOf("")}
    val valid=roomId>0&&name.isNotBlank()&&PhoneRules.isCompleteTail(phoneTail)&&rent.asLong()>0&&due.toIntOrNull() in 1..31&&runCatching{LocalDate.parse(start)<=LocalDate.parse(end)}.getOrDefault(false)
    AlertDialog(onDismissRequest=onDismiss,title={Text("辦理入住")},text={Column(Modifier.heightIn(max=560.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(7.dp)){if(rooms.isEmpty())Text("目前沒有空房") else ExposedDropdownMenuBox(expanded,{expanded=!expanded}){OutlinedTextField(selected?.let{"${it.venueName} · ${it.name}"}?:"選擇房間",{},readOnly=true,label={Text("房間")},trailingIcon={ExposedDropdownMenuDefaults.TrailingIcon(expanded)},modifier=Modifier.menuAnchor().fillMaxWidth());ExposedDropdownMenu(expanded,{expanded=false}){rooms.forEach{r->DropdownMenuItem({Text("${r.venueName} · ${r.name}")},{roomId=r.id;rent=r.defaultRent.toString();expanded=false})}}};FormField(name,{name=it},"房客姓名");MobilePhoneField(phoneTail,{phoneTail=it});FormField(line,{line=it},"LINE 顯示名稱");Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){DatePickerField("起租日",start,{start=it},Modifier.weight(1f));DatePickerField("到期日",end,{end=it},Modifier.weight(1f))};Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FormField(rent,{rent=it},"月租",Modifier.weight(1f),true);FormField(due,{due=it},"繳租日",Modifier.weight(1f),true)};FormField(deposit,{deposit=it},"押金",numeric=true);Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FormField(waterFee,{waterFee=it},"每月水費",Modifier.weight(1f),true);FormField(managementFee,{managementFee=it},"每月管理費",Modifier.weight(1f),true)};FormField(electricityFee,{electricityFee=it},"每月電費",numeric=true);FormField(note,{note=it},"租約備註")}},dismissButton={TextButton(onClick=onDismiss){Text("取消")}},confirmButton={Button(enabled=valid,onClick={onSave(Tenant(name=name.trim(),phone=PhoneRules.canonical(phoneTail),lineName=line.trim()),Lease(roomId=roomId,tenantId=0,startDate=start,endDate=end,monthlyRent=rent.asLong(),dueDay=due.toInt(),deposit=deposit.asLong(),waterFee=waterFee.asLong(),managementFee=managementFee.asLong(),electricityFee=electricityFee.asLong(),note=note.trim()))}){Text("建立租約")}})
}

@Composable private fun TenantDialog(value:Tenant,onDismiss:()->Unit,onSave:(Tenant)->Unit){var name by remember{mutableStateOf(value.name)};var phoneTail by remember{mutableStateOf(PhoneRules.tailFromStored(value.phone))};var line by remember{mutableStateOf(value.lineName)};var note by remember{mutableStateOf(value.note)};AlertDialog(onDismissRequest=onDismiss,title={Text("編輯房客")},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)){FormField(name,{name=it},"姓名");MobilePhoneField(phoneTail,{phoneTail=it});FormField(line,{line=it},"LINE 顯示名稱");FormField(note,{note=it},"備註")}},dismissButton={TextButton(onClick=onDismiss){Text("取消")}},confirmButton={Button(enabled=name.isNotBlank()&&PhoneRules.isCompleteTail(phoneTail),onClick={onSave(value.copy(name=name.trim(),phone=PhoneRules.canonical(phoneTail),lineName=line.trim(),note=note.trim()))}){Text("儲存")}})}
...
@Composable private fun EditLeaseDialog(value:LeaseListRow,onDismiss:()->Unit,onSave:(Lease)->Unit){var start by remember{mutableStateOf(value.startDate)};var end by remember{mutableStateOf(value.endDate)};var rent by remember{mutableStateOf(value.monthlyRent.toString())};var due by remember{mutableStateOf(value.dueDay.toString())};var deposit by remember{mutableStateOf(value.deposit.toString())};var waterFee by remember{mutableStateOf(value.waterFee.toString())};var managementFee by remember{mutableStateOf(value.managementFee.toString())};var electricityFee by remember{mutableStateOf(value.electricityFee.toString())};var note by remember{mutableStateOf(value.note)};val valid=rent.asLong()>0&&due.toIntOrNull() in 1..31&&runCatching{LocalDate.parse(start)<=LocalDate.parse(end)}.getOrDefault(false);AlertDialog(onDismissRequest=onDismiss,title={Text("修改租約 · ${value.roomName}")},text={Column(Modifier.heightIn(max=560.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("房客：${value.tenantName}");Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){DatePickerField("起租日",start,{start=it},Modifier.weight(1f));DatePickerField("到期日",end,{end=it},Modifier.weight(1f))};Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FormField(rent,{rent=it},"月租",Modifier.weight(1f),true);FormField(due,{due=it},"繳租日",Modifier.weight(1f),true)};FormField(deposit,{deposit=it},"押金",numeric=true);Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FormField(waterFee,{waterFee=it},"每月水費",Modifier.weight(1f),true);FormField(managementFee,{managementFee=it},"每月管理費",Modifier.weight(1f),true)};FormField(electricityFee,{electricityFee=it},"每月電費",numeric=true);FormField(note,{note=it},"租約備註");Text("修改租約不會自動更動已產生的帳單。",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)}},dismissButton={TextButton(onClick=onDismiss){Text("取消")}},confirmButton={Button(enabled=valid,onClick={onSave(Lease(id=value.id,roomId=value.roomId,tenantId=value.tenantId,startDate=start,endDate=end,monthlyRent=rent.asLong(),dueDay=due.toInt(),deposit=deposit.asLong(),waterFee=waterFee.asLong(),managementFee=managementFee.asLong(),electricityFee=electricityFee.asLong(),note=note.trim(),status=value.status,endedAt=value.endedAt))}){Text("儲存修改")}})}
