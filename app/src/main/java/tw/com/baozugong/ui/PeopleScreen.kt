...
fun PeopleScreen(vm:AppViewModel,onMessage:(String)->Unit) {
    ...
    var newLease by remember { mutableStateOf(false) };var editTenant by remember { mutableStateOf<Tenant?>(null) };var renew by remember { mutableStateOf<LeaseListRow?>(null) };var confirmEnd by remember { mutableStateOf<LeaseListRow?>(null) }
    ...
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(bottom=24.dp)) {
        ...
        if(!showTenants) {
            ...
            items(active,key={it.id}){lease->LeaseCard(lease,onCall={if(lease.tenantPhone.isNotBlank())context.startActivity(Intent(Intent.ACTION_DIAL,Uri.parse("tel:${lease.tenantPhone}")))},onRenew={renew=lease},onEnd={confirmEnd=lease})}
            if(history.isNotEmpty()){item{Text("歷史租約",Modifier.padding(16.dp),fontWeight=FontWeight.Bold)};items(history,key={it.id}){LeaseCard(it,null,null,null)}}
        } else {
