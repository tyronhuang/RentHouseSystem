...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            AndroidBaoZuGongTheme(darkTheme) { MainApp(vm, darkTheme, onThemeChange = { darkTheme = it }) }
        }
    }
}

@Composable
private fun AndroidBaoZuGongTheme(darkTheme: Boolean, content: @Composable () -> Unit) {
...
private fun MainApp(vm: AppViewModel, darkTheme: Boolean, onThemeChange: (Boolean) -> Unit) {
    ...
    Scaffold(
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when(tab) {
                MainTab.HOME -> DashboardScreen(vm.controller, onOpenBills={tab=MainTab.BILLS})
                MainTab.ROOMS -> RoomsScreen(vm.controller, onMessage={message=it})
                MainTab.BILLS -> BillsScreen(vm.controller, onMessage={message=it}, onShare={ text -> context.startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type="text/plain";putExtra(Intent.EXTRA_TEXT,text) },"分享催繳訊息")) })
                MainTab.PEOPLE -> PeopleScreen(vm.controller, onMessage={message=it}, onCallPhone={phone->context.startActivity(Intent(Intent.ACTION_DIAL,Uri.parse("tel:$phone")))})
