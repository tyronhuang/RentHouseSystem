...
@Composable
fun DashboardScreen(vm: AppController,onOpenBills:()->Unit) {
    ...
    val today=currentDate();val current=invoices.filter { it.billingMonth==currentMonth() && it.rawStatus!=InvoiceStatus.VOID && it.paid<it.total }
    val expiring=leases.filter { it.status=="ACTIVE" && daysBetween(today,it.endDate) in 0..30 }
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(bottom=24.dp)) {
        item { ScreenTitle("本月總覽",currentMonth()) }
        ...
        if(current.isEmpty()) item { EmptyHint("本月沒有待收帳款") } else items(current.take(10),key={it.id}) { bill ->
            ListItem(headlineContent={Text("${bill.venueName} · ${bill.roomName}")},supportingContent={Text("${bill.tenantName}　到期 ${bill.dueDate}")},trailingContent={Column{Text(money((bill.total-bill.paid).coerceAtLeast(0)),fontWeight=FontWeight.Bold);Text(bill.displayStatus(today),color=if(bill.dueDate<today)MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.primary)}},modifier=Modifier.clickable(onClick=onOpenBills));HorizontalDivider()
        }
