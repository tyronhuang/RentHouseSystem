...
@Composable private fun InvoiceDialog(vm:AppController,bill:InvoiceListRow,onDismiss:()->Unit,onMessage:(String)->Unit,onShare:(String)->Unit) {
    ...
    AlertDialog(onDismissRequest=onDismiss,title={Text("${bill.billingMonth} · ${bill.roomName}")},text={LazyColumn(Modifier.heightIn(max=520.dp)){item{Text("${bill.venueName}　${bill.tenantName}");Text("到期日 ${bill.dueDate}");Spacer(Modifier.height(12.dp));Text("帳單項目",fontWeight=FontWeight.Bold)};items(chargeItems,key={it.id}){item->ListItem(headlineContent={Text(item.title)},trailingContent={Text(money(item.amount))},modifier=Modifier.clickable(enabled=paid==0L&&bill.rawStatus!=InvoiceStatus.VOID){editItem=item})};item{HorizontalDivider();Text("收款紀錄",Modifier.padding(top=12.dp),fontWeight=FontWeight.Bold)};if(payments.isEmpty())item{Text("尚無收款",Modifier.padding(vertical=12.dp),color=MaterialTheme.colorScheme.onSurfaceVariant)} else items(payments,key={it.id}){p->ListItem(headlineContent={Text("${p.paidDate} · ${p.method}")},supportingContent={if(p.note.isNotBlank())Text(p.note)},trailingContent={Text(money(p.amount))},modifier=Modifier.clickable{editPayment=p})};item{HorizontalDivider();Row(Modifier.fillMaxWidth().padding(vertical=12.dp),horizontalArrangement=Arrangement.SpaceBetween){Text("尚未收",fontWeight=FontWeight.Bold);Text(money(remaining),fontWeight=FontWeight.Bold,color=if(remaining>0)MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.primary)}}}},dismissButton={TextButton(onClick=onDismiss){Text("關閉")}},confirmButton={Column(horizontalAlignment=androidx.compose.ui.Alignment.End){Row{TextButton(enabled=bill.rawStatus!=InvoiceStatus.VOID,onClick={onShare("您好，${bill.billingMonth} ${bill.venueName} ${bill.roomName} 應繳 ${money(total)}，目前尚有 ${money(remaining)} 未繳，繳款期限為 ${bill.dueDate}，再請協助確認，謝謝。")}){Text("分享催繳")};TextButton(enabled=paid==0L&&bill.rawStatus!=InvoiceStatus.VOID,onClick={addCharge=true}){Text("＋費用")}};Row{TextButton(enabled=paid==0L&&bill.rawStatus!=InvoiceStatus.VOID,onClick={confirmVoid=true}){Text("作廢")};Button(enabled=remaining>0&&bill.rawStatus!=InvoiceStatus.VOID,onClick={addPayment=true}){Text("登記收款")}}}})
    ...
    if(addPayment) PaymentDialog(null,remaining,{addPayment=false}){amount,date,method,note->vm.run{addPayment(bill.id,amount,date,method,note)};addPayment=false}
    if(editPayment!=null) PaymentDialog(editPayment,remaining+editPayment!!.amount,{editPayment=null}){amount,date,method,note->vm.run{updatePayment(editPayment!!.copy(amount=amount,paidDate=date,method=method,note=note))};editPayment=null}
